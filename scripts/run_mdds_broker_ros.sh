...
if [[ "$cli_batch" == graph_late || "$cli_batch" == graph_off || "$peer_restart" == 1 ]]; then "$GRAPH_HOST_PYTHON" scripts/mdds_e2e/late_graph_hashes.py "$LOGDIR"; fi
...
for board in "$BOARD_A" "$BOARD_B"; do
  ...
  if [[ "$cli_batch" == service_qos || "$cli_batch" == endpoint_qos || "$cli_batch" == graph_duplicate || "$cli_batch" == graph_churn || "$cli_batch" == graph_abrupt || "$cli_batch" == graph_off ]]; then
...
for phase in 1 2; do
  ...
  if [[ "$phase" == 1 ]]; then
    ...
    if [[ "$cli_batch" != none ]]; then
      ...
      if [[ "$cli_batch" == graph_hidden || "$cli_batch" == graph_duplicate || "$cli_batch" == graph_churn || "$cli_batch" == graph_abrupt || "$cli_batch" == graph_off ]]; then
        wait_hidden_marker() {
          local board="$1" name="$2" ready=false value attempt maximum=120
          if [[ "$cli_batch" == graph_churn || "$cli_batch" == graph_abrupt || "$cli_batch" == graph_off ]]; then maximum=450; fi
