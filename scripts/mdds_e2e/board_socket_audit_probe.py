...
if len(sys.argv)==6:
    if mode!='missing':os.environ['LD_PRELOAD']=' '.join(filter(None,[os.environ.get('LD_PRELOAD'),str(library)]))
...
import numpy
numpy_control=numpy.zeros(2,dtype='uint8').tolist();assert numpy_control==[0,0]
calls=list(early)
...
record={'run_id':run,'nonce':nonce,'board':board,'mode':mode,'pid':os.getpid(),'start':process_start(os.getpid()),'argv':Path('/proc/self/cmdline').read_bytes().rstrip(b'\0').decode().split('\0'),
        ...
        'numpy_control':numpy_control,'preload':os.environ.get('LD_PRELOAD','').split()}
