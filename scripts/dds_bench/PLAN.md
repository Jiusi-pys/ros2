...
User-approved scope: MDDS/DSoftBus, Fast DDS, Cyclone DDS; BEST_EFFORT and
RELIABLE; A->B, B->A and simultaneous bidirectional traffic; 1 KiB through
4 MiB regular payloads and a separate 8..512 MiB boundary ladder.
...
RED -> cross-build GREEN -> two-board deployment SHA check -> 3 RMW x 2 QoS
...
Bounded execution: max payload 512 MiB, max samples 1 million, outer wall-clock
deadline, per-board memory monitoring (application + owned MDDS broker), process
