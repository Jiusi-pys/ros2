...
MddsLoanMemoryResource::MddsLoanMemoryResource(void *storage, size_t capacity)
: arena_(storage, capacity)
{
  resource_.state = this;
  resource_.allocate = &MddsLoanMemoryResource::AllocateCallback;
  resource_.deallocate = &MddsLoanMemoryResource::DeallocateCallback;
}

void * MddsLoanMemoryResource::Allocate(size_t size, size_t alignment)
{
  return arena_.Allocate(size, alignment);
}

bool MddsLoanMemoryResource::Contains(const void *data, size_t size) const
{
  return arena_.Contains(data, size);
}

size_t MddsLoanMemoryResource::BytesUsed() const
{
  return arena_.BytesUsed();
}

size_t MddsLoanMemoryResource::SegmentCount() const
{
  return arena_.SegmentCount();
}

const rosidl_runtime_cpp::MessageMemoryResource *
MddsLoanMemoryResource::Resource() const
{
  return &resource_;
}

void * MddsLoanMemoryResource::AllocateCallback(
  ...
  size_t alignment)
{
  ...
  return static_cast<MddsLoanMemoryResource *>(state)->Allocate(size,
                                                                alignment);
}

void MddsLoanMemoryResource::DeallocateCallback(
  void *state, void *pointer,
  size_t size,
  size_t alignment)
{
  (void)state;
