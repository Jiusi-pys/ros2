...
void ExpectValidGraphUpdate(const rmw_mdds_cpp::ipc::Frame &frame) {
  ...
  ASSERT_TRUE(rmw_mdds_cpp::ipc::DecodeEndpointList(
...
TEST(RmwMddsIpcBroker, DoesNotRouteBestEffortSamplesToReliableSubscription) {
  ...
  auto best_effort_endpoint =
      MakeEndpoint(5200u, rmw_mdds_cpp::ipc::EndpointKind::kSubscription,
                   "/broker/qos_chatter", "std_msgs/msg/String");
  best_effort_endpoint.qos.reliability =
...
TEST(RmwMddsIpcBroker, RelaysPeerBrokerGraphOverBridgeSync) {
  ...
  TempSocketPath socket_path_a;
  TempSocketPath socket_path_b;
  ASSERT_FALSE(socket_path_a.path().empty());
  ASSERT_FALSE(socket_path_b.path().empty());

  std::string error;
  rmw_mdds_cpp::ipc::IpcBroker broker_a;
  rmw_mdds_cpp::ipc::IpcBroker broker_b;
  ASSERT_TRUE(broker_a.Start(socket_path_a.path(), &error)) << error;
  ASSERT_TRUE(broker_b.Start(socket_path_b.path(), &error)) << error;

  rmw_mdds_cpp::ipc::UniqueFd observer_a =
      rmw_mdds_cpp::ipc::ConnectUnixSocket(socket_path_a.path(), &error);
  ASSERT_TRUE(observer_a) << error;
  DrainReadableFrames(observer_a.get(), std::chrono::milliseconds(100));

  rmw_mdds_cpp::ipc::UniqueFd publisher_b =
      rmw_mdds_cpp::ipc::ConnectUnixSocket(socket_path_b.path(), &error);
  ASSERT_TRUE(publisher_b) << error;
  RegisterEndpoint(publisher_b.get(),
                   MakeEndpoint(1200u,
                                rmw_mdds_cpp::ipc::EndpointKind::kPublisher,
  ...
  EXPECT_TRUE(WaitForGraphEndpoint(
      observer_a.get(), rmw_mdds_cpp::ipc::EndpointKind::kPublisher,
      "/broker/peer_graph", "std_msgs/msg/String", std::chrono::seconds(3)))
      << "broker A must rebroadcast broker B's graph-sync publisher endpoint";
}
...
TEST(RmwMddsIpcBroker, OwnsBridgeTransportForRegisteredServiceEndpoints) {
  ...
  EXPECT_TRUE(ReadUntilPayload(service.get(), request.payload, &local_request,
                               &error))
      << error;
  ...
  EXPECT_TRUE(ReadUntilPayload(client.get(), response.payload, &local_response,
                               &error))
      << error;
...
TEST(RmwMddsIpcBroker,
     SynthesizesRemoteServiceGraphFromResponsePublisherMatch) {
  EnvVarGuard bridge_guard("RMW_MDDS_BRIDGE_LIBRARY");
  rmw_mdds_cpp::BridgeBackend::Instance().ResetForTesting();
  ASSERT_EQ(0, setenv("RMW_MDDS_BRIDGE_LIBRARY", FAKE_MDDS_BRIDGE_PATH, 1));
  FakeMddsBridgeReset();
  ...
  EXPECT_TRUE(WaitForGraphEndpoint(
      ...
      "/response_matched_trigger", "std_srvs/srv/Trigger",
      std::chrono::seconds(1)))
      << "a matched remote response publisher also proves a remote service "
         "server";
