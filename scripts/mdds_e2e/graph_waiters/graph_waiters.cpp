  ...
  const std::string & root)
{
...
#ifdef GRAPH_REMOTE_WAITER
  const std::string target_space = "/remote_guard_" + run_id + "/" + peer;
  constexpr auto graph_timeout = 5s;
  std::cout << "GRAPH_WAITER_SCOPE remote" << std::endl;
#else
  const std::string target_space = space;
  constexpr auto graph_timeout = 2s;
  (void)root;
#endif
  ...
  const auto phase = [&](const std::string & name, const std::function<void()> & change,
      ...
      for (int attempt = 0; attempt < 5; ++attempt) {
        ...
        auto a = std::async(std::launch::async, [&] {
          entered.fetch_add(1); first->wait_for_graph_change(event_a, graph_timeout); return event_a->check();
        });
        auto b = std::async(std::launch::async, [&] {
          entered.fetch_add(1); second->wait_for_graph_change(event_b, graph_timeout); return event_b->check();
        });
...
#ifdef GRAPH_REMOTE_WAITER
        const std::string ready = root + "/remote_wait_" + std::to_string(phase_index) + ".ready";
        {std::ofstream stream(ready + ".pending"); stream << nonce << std::endl;}
        if (std::rename((ready + ".pending").c_str(), ready.c_str()) != 0) throw std::runtime_error("cannot arm remote phase");
        std::cout << "GRAPH_REMOTE_ARMED " << phase_index << " " << nonce << std::endl;
        const auto deadline = std::chrono::steady_clock::now() + 8s;
        std::string applied;
        while (std::chrono::steady_clock::now() < deadline) {
          std::ifstream stream(root + "/remote_applied_" + std::to_string(phase_index) + ".done");
          std::getline(stream, applied);
          if (applied == nonce) break;
          std::this_thread::sleep_for(10ms);
        }
        if (applied != nonce) throw std::runtime_error("remote phase not applied");
#else
        change();
#endif
        const bool ready_a = a.get(), ready_b = b.get();
        bool snapshot = check(*first) && check(*second);
#ifdef GRAPH_REMOTE_WAITER
        const auto converge = std::chrono::steady_clock::now() + 3s;
        while (!snapshot && std::chrono::steady_clock::now() < converge) {
          std::this_thread::sleep_for(10ms); snapshot = check(*first) && check(*second);
        }
#endif
  ...
  const std::string topic = target_space + "/topic", service = target_space + "/service";
  ...
  const auto has_added = [&](rclcpp::Node & node) {
      ...
      return std::count(names.begin(), names.end(), target_space + "/added");
    };
...
int main(int argc, char ** argv)
{
  ...
  int result = 0;
  try {run(argv[1], argv[2], argv[3], root);}
