...
def emit(root,manifest,reports,run,nonce):
    ...
    if flag.read_text().strip()!=CASE or (root/'cli_batch').read_text().strip()!='service_qos':
        raise ValueError('unsupported graph case request')
    case=next(c for c in manifest['cases'] if c['id']==CASE)
    executions=[]
    for board in a.TARGET['board_serials']:
        validate(reports[board],root,run,board,nonce)
        ...
        for marker in ('MDDS_GRAPH_ACTUAL_ARGV '+json.dumps(argv),a.terminal_marker(run,CASE,0,argv,board),
    ...
    receipt={'schema_version':1,'run_id':run,'case_id':CASE,'kind':'functional','status':'PASS',
             ...
             'executions':executions,'assertions':[{'id':name,'passed':True,'execution':0,
                 'pattern':'SERVICE_QOS_RESULT '+json.dumps(reports[a.TARGET['board_serials'][0]]['service_qos'])} for name in case['assertions']]}
    names=['graph_case','host_report.json']+[board+'.'+suffix for board in reports for suffix in ('service_qos.json','service_qos_server.json','ros.status.json','daemon.log','daemon.inspect.json')]
    receipt['graph_provenance']=[{'path':name,'sha256':a.digest((root/name).read_bytes())} for name in names]
    path=root/'graph_service_client_ownership.receipt.json';path.write_text(json.dumps(receipt,indent=2)+'\n')
