...
def validate(root, run, a, b):
    ...
    try:
        ...
        require(policy_mode in ('explicit', 'implicit','selector'), 'invalid policy mode')
        ...
        for board, peer, label in ((a, b, 'A'), (b, a, 'B')):
            ...
            for v in provenance:
                require(v.get('transport_policy') == {'mode': policy_mode, 'profile': 'ohos_dsoftbus' if policy_mode == 'explicit' else None, 'selector': 'dsoftbus' if policy_mode=='selector' else None, 'discovery_range': 'SYSTEM_DEFAULT'}, 'wrong actual transport-policy environment')
