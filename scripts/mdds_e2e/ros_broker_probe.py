...
try:
    ...
    if (root/'cli_batch').read_text().strip()=='graph_domain':
        ...
        from board_duplicate_graph import DuplicateGraphSource
        component_probe=DuplicateGraphSource(root,a.run_id,a.nonce,a.role,records[0]['node'])
    elif (root/'cli_batch').read_text().strip()=='graph_hidden':
        ...
        from board_late_source import LateGraphSource
        component_probe=LateGraphSource(root,a.run_id,a.nonce,a.role,records[0]['node'],records[0]['executor'])
    elif (root/'cli_batch').read_text().strip()=='graph_remote':
        ...
        component_probe=StatisticsProbe(root,a.run_id,a.nonce,a.self_serial,a.peer_serial,a.role,records[0]['node'])
    elif (root/'cli_batch').read_text().strip()=='components':
    ...
    if component_probe is not None and hasattr(component_probe,'close'):component_probe.close()
    for r in records:
        for key in ('action_client','action_server'):
