...
PY
  ...
  verify_command="$PY_CRYPTO_ENV LD_LIBRARY_PATH='$PY_PREFIX/lib' LD_PRELOAD='$PY_PREFIX/lib/libpython3.12.so.1.0' '$PY_BIN' -I -B -c 'import os,platform,sys,sysconfig; sys.path.insert(0,\"$remote_stage\"); import numpy,yaml,psutil,lark,catkin_pkg,rosdistro,rospkg,distro,argcomplete,packaging,setuptools,pip,em,lxml.etree,cryptography.fernet,cffi,pycparser,pytest,pytest_timeout,pytest_repeat,pytest_rerunfailures,pytest_mock,colcon_core,colcon_cmake,colcon_ros,colcon_test_result,colcon_python_setup_py; overlay=os.path.realpath(\"$remote_stage\")+\"/\"; assert all(os.path.realpath(m.__file__).startswith(overlay) for m in (numpy,yaml,psutil,lark,catkin_pkg,rosdistro,rospkg,distro)); assert sys.version_info[:3] == (3,12,7); assert sysconfig.get_config_var(\"SOABI\") == \"cpython-312-aarch64-linux-ohos\"; assert platform.machine() in (\"aarch64\",\"arm64\"); print(\"PYTHON_DEPS_OK version=3.12.7 soabi=cpython-312-aarch64-linux-ohos arch=\" + platform.machine() + \" numpy=\" + numpy.__version__)'"
