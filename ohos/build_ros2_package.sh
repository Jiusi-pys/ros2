...
if [[ -d "${TARGET_PYTHON_INCLUDE_DIR}" && -f "${TARGET_PYTHON_LIBRARY}" && -d "${TARGET_NUMPY_INCLUDE_DIR}" ]]; then
  package_dir_args+=(
    ...
    # rclpy's OHOS adaptation: when these target-python hints are set it does
    # find_package(Python3 Interpreter) only, skipping the Development component
    # that cannot be satisfied for an aarch64/musl target from an x86 host.
    ...
    "-DRCLPY_OHOS_TARGET_PYBIND11_INCLUDE_DIR=${PREFIX}/include"
  )
fi
