...
cp scripts/mdds_e2e/{cli_graph_basic,cli_graph_lists,cli_daemon,cli_daemon_guard,cli_acceptance}.py "$LOGDIR/"
...
for board in "$BOARD_A" "$BOARD_B"; do
  ...
  for name in mdds_broker_daemon mdds_token_exec board_graph_ownership.py broker_local_run.py ros_broker_supervise.py mdds_broker_service.py libmdds.so librmw_mdds.so ros_broker_probe.py broker_local_ros_probe.py type_description_lifetime.py profile.env rclpy_overlay.tar rclpy_package.json type_hashes.json policy_mode cli_batch cli_graph_basic.py cli_graph_lists.py cli_daemon.py cli_daemon_guard.py cli_acceptance.py cli_acceptance_manifest.json; do
...
for board in "$BOARD_A" "$BOARD_B"; do
  ...
  if [[ "$cli_batch" == basic ]]; then
    ...
    for name in status_before start status_running nodes_cached nodes_direct stop status_after nodes_after_stop; do
