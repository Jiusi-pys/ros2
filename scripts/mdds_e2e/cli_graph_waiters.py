...
def is_remote(root):return (root/'cli_batch').read_text().strip()=='graph_remote'
def case_id(root):return 'graph:remote_guard' if is_remote(root) else CASE


def output_contract(raw,run,role,nonce,remote=False):
    ...
    if remote:
        if raw.splitlines().count('GRAPH_WAITER_SCOPE remote')!=1:raise ValueError('local graph mutation substituted for remote')
        for index in range(len(PHASES)):
            if raw.splitlines().count(f'GRAPH_REMOTE_ARMED {index} {nonce}')!=1:raise ValueError('remote wait admission missing')
    return rows
...
def execute(root,run,board,nonce):
    ...
    remote=is_remote(root);argv=[str(root/'graph_waiters'),run,role,nonce]+([str(root)] if remote else []);native=None;emergency=False
    ...
    log.write_text('MDDS_GRAPH_ACTUAL_ARGV '+json.dumps(argv)+'\n'+raw+'\n'+a.terminal_marker(run,case_id(root),child.returncode,argv,board)+'\n')
    ...
    output_contract(raw,run,role,nonce,remote)
...
def validate(value,root,run,board,nonce):
    ...
    remote_mode=is_remote(root)
    wanted={'run_id':run,'board':board,'role':role,'nonce':nonce,'argv':[remote+'/graph_waiters',run,role,nonce]+([remote] if remote_mode else []),'returncode':0,'emergency_cleanup':False}
    ...
    output_contract(raw,run,role,nonce,remote_mode)
    ...
    if remote_mode:
        from remote_graph_contract import validate_change
        for index in range(len(PHASES)):
            for owner,name in ((board,f'remote_wait_{index}.ready'),(peer,f'remote_change_{index}.go'),(peer,f'remote_source_{index}.done'),(board,f'remote_applied_{index}.done')):
                if (root/(owner+'.'+name)).read_text().strip()!=nonce:raise ValueError('remote graph phase barrier differs')
            change=json.loads((root/(peer+f'.remote_source_{index}.json')).read_bytes())
            validate_change(change,run,nonce,role,index)
            if (root/(peer+'.ros.log')).read_text().splitlines().count('GRAPH_REMOTE_SOURCE '+json.dumps(change))!=1:raise ValueError('remote source operation lacks raw log')


def emit_receipt(root,manifest,reports,run,nonce):
    case_name=case_id(root);case=next(c for c in manifest['cases'] if c['id']==case_name)
    executions=[]
    ...
    receipt={'schema_version':1,'run_id':run,'case_id':case_name,'kind':'functional','status':'PASS',
             ...
             'assertions':[{'id':('remote_' if is_remote(root) else '')+kind+'_wakes','passed':True,'execution':0,
    ...
    if is_remote(root):
        names += [board+'.'+suffix for board in reports for index in range(len(PHASES)) for suffix in (f'remote_wait_{index}.ready',f'remote_change_{index}.go',f'remote_source_{index}.done',f'remote_applied_{index}.done',f'remote_source_{index}.json')]
