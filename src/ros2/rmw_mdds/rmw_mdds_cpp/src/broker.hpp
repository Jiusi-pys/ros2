...
struct PublisherData {
  ...
  // Offered-deadline QoS enforcement (lazy poll accounting; see broker.cpp
  // deadline helpers).
  ...
  // Offered-QoS-incompatible: raised when a matched subscription requests a QoS
  // this publisher's offer cannot satisfy (detected at match time via
  // rmw_dds_common compatibility check).
...
struct SubscriptionData {
  ...
  // Requested-deadline QoS enforcement (lazy poll accounting; reset on sample
  // arrival).
  ...
  // Liveliness-changed QoS (AUTOMATIC): alive_count == currently-matched
  // publisher count.
  ...
  // Requested-QoS-incompatible: raised when matched to a publisher offering an
  // incompatible QoS.
  ...
  // Message-lost: detected from gaps in each writer's publication sequence
  // numbers. In-process delivery is lossless (count stays 0); gaps occur on the
  // lossy RTPS / bridge ingress path.
  ...
  std::map<std::array<uint8_t, RMW_GID_STORAGE_SIZE>, uint64_t>
  ...
  std::vector<std::vector<StringContentFilterClause>>
      string_filter_disjunctions;
  // Numeric content filter (`field OP number`, DDS-SQL subset) for non-String
  // message types, evaluated against the introspected field. Mutually exclusive
  // with the std_msgs/String string filter above: at most one of
  // content_filter_enabled / numeric_filter_enabled is set.
  ...
  std::vector<std::vector<NumericContentFilterClause>>
...
void PublishToSubscriptions(PublisherData *publisher,
...
size_t EnqueueRtpsUserDataForReader(const rtps::EntityId &reader_id,
                                    ...
                                    int64_t writer_sequence_number,
                                    const std::vector<uint8_t> &payload);
size_t EnqueueRtpsUserDataForTopic(const std::string &dds_topic_name,
                                   const rtps::GuidPrefix &writer_guid_prefix,
                                   const rtps::EntityId &writer_id,
                                   int64_t writer_sequence_number,
                                   const std::vector<uint8_t> &payload);
...
bool TakePublisherMatchedStatus(PublisherData *publisher,
                                rmw_matched_status_t *status);
bool TakeSubscriptionMatchedStatus(SubscriptionData *subscription,
                                   rmw_matched_status_t *status);
...
size_t SetPublisherMatchedCallback(PublisherData *publisher,
                                   rmw_event_callback_t callback,
                                   const void *user_data);
size_t SetSubscriptionMatchedCallback(SubscriptionData *subscription,
                                      rmw_event_callback_t callback,
                                      const void *user_data);
size_t SetSubscriptionNewMessageCallback(SubscriptionData *subscription,
                                         rmw_event_callback_t callback,
                                         const void *user_data);

// Offered / requested QoS-incompatible events. Counts and last-incompatible
// policy kind are accrued at match time (RegisterPublisher /
// RegisterSubscription) by an rmw_dds_common compatibility check.
...
size_t SetPublisherQosIncompatibleCallback(PublisherData *publisher,
                                           rmw_event_callback_t callback,
                                           const void *user_data);
size_t SetSubscriptionQosIncompatibleCallback(SubscriptionData *subscription,
                                              rmw_event_callback_t callback,
                                              const void *user_data);

// Offered / requested incompatible-type events. Counts are accrued when
// endpoints share a topic name but advertise different ROS type names, so they
// are visible even though the endpoints never match.
...
bool HasUnreadSubscriptionIncompatibleTypeStatus(
    SubscriptionData *subscription);
size_t SetPublisherIncompatibleTypeCallback(PublisherData *publisher,
                                            rmw_event_callback_t callback,
                                            const void *user_data);
size_t SetSubscriptionIncompatibleTypeCallback(SubscriptionData *subscription,
                                               rmw_event_callback_t callback,
                                               const void *user_data);

// Message-lost event: detected from gaps in each writer's publication sequence
// numbers on ingress.
bool TakeSubscriptionMessageLostStatus(SubscriptionData *subscription,
                                       rmw_message_lost_status_t *status);
bool HasUnreadSubscriptionMessageLostStatus(SubscriptionData *subscription);
size_t SetSubscriptionMessageLostCallback(SubscriptionData *subscription,
                                          rmw_event_callback_t callback,
                                          const void *user_data);
...
// Liveliness-changed QoS (AUTOMATIC). The current alive (matched-publisher)
// count is supplied by the caller so it can use the broker-graph or local
// registry as appropriate.
bool HasUnreadSubscriptionLivelinessStatus(SubscriptionData *subscription,
                                           size_t alive_count);
bool TakeSubscriptionLivelinessStatus(SubscriptionData *subscription,
                                      size_t alive_count,
                                      rmw_liveliness_changed_status_t *status);
size_t SetSubscriptionLivelinessCallback(SubscriptionData *subscription,
                                         size_t alive_count,
                                         rmw_event_callback_t callback,
                                         const void *user_data);

// Deadline QoS enforcement (lazy poll accounting up to now_ns; no monitor
// thread). NoteSubscriptionSampleArrival / NotePublisherPublication reset the
// active reference so on-time traffic never registers a miss. `matched` gates
// requested-deadline misses.
void NoteSubscriptionSampleArrival(SubscriptionData *subscription,
                                   int64_t now_ns);
...
void NoteNodePublishersLivelinessAsserted(const char *node_name,
                                          const char *node_namespace,
                                          int64_t now_ns);
bool HasUnreadPublisherLivelinessLostStatus(PublisherData *publisher,
                                            int64_t now_ns);
bool TakePublisherLivelinessLostStatus(PublisherData *publisher, int64_t now_ns,
                                       rmw_liveliness_lost_status_t *status);
size_t SetPublisherLivelinessLostCallback(PublisherData *publisher,
                                          int64_t now_ns,
                                          rmw_event_callback_t callback,
                                          const void *user_data);
bool HasUnreadSubscriptionDeadlineStatus(SubscriptionData *subscription,
                                         int64_t now_ns, bool matched);
...
size_t SetSubscriptionDeadlineCallback(SubscriptionData *subscription,
                                       int64_t now_ns, bool matched,
                                       rmw_event_callback_t callback,
                                       const void *user_data);
bool HasUnreadPublisherDeadlineStatus(PublisherData *publisher, int64_t now_ns);
bool TakePublisherDeadlineStatus(PublisherData *publisher, int64_t now_ns,
                                 rmw_offered_deadline_missed_status_t *status);
size_t SetPublisherDeadlineCallback(PublisherData *publisher, int64_t now_ns,
                                    rmw_event_callback_t callback,
                                    const void *user_data);
...
GetPublisherNamesAndTypesByNode(const char *node_name,
                                const char *node_namespace);
std::vector<NameAndTypes>
GetSubscriptionNamesAndTypesByNode(const char *node_name,
                                   const char *node_namespace);
std::vector<TopicEndpointInfo>
GetPublisherEndpointInfosByTopic(const char *topic_name);
std::vector<TopicEndpointInfo>
GetSubscriptionEndpointInfosByTopic(const char *topic_name);
