...
def pack(root):
    source=Path.cwd()/'src/ros2/ros2cli/ros2cli/ros2cli';files=[];archive=root/'ros2cli_overlay.zip'
    with zipfile.ZipFile(archive,'w',zipfile.ZIP_DEFLATED) as zipped:
        for path in sorted(source.rglob('*.py')):
            if '__pycache__' in path.parts:continue
            if path.is_symlink():raise ValueError('linked CLI source')
            name='ros2cli/'+path.relative_to(source).as_posix();raw=path.read_bytes()
            files.append({'path':name,'sha256':digest(raw),'size':len(raw)});zipped.writestr(name,raw)
    manifest={'schema_version':1,'archive_sha256':digest(archive.read_bytes()),'files':files}
...
def prepare(root,expected):
    ...
    for item in manifest['files']:
        path=site/item['path']
        if not path.resolve().is_relative_to((site/'ros2cli').resolve()) or path.stat().st_size!=item['size'] or digest(path.read_bytes())!=item['sha256']:raise ValueError('CLI overlay source differs')
