...
8. **重建集合必含"消息集变化的接口 repo"（2026-06-15 实测踩坑）**：jazzy 是稳定分支，**ABI 只增不减**——纯 C++ 库即使没重建、用旧产物也照常工作（这正是 129/130 仍能过的原因）。唯一会炸的是**接口包新增了 `.msg`/`.srv`/`.action`**：新消息会让 typesupport 导出新符号。本次 `common_interfaces` 在 `PULL_ONLY`（只拉不重建）里，但 jazzy 给 `geometry_msgs` 新增了 `VelocityStamped`/`VelocityWithCovarianceStamped`（commit `ac9fc9b`）→ 新 staging 的 Python typesupport 扩展引用 `rosidl_typesupport_c__get_message_type_support_handle__geometry_msgs__msg__VelocityWithCovarianceStamped`，而旧的 `libgeometry_msgs__rosidl_typesupport_c.so` 没这个符号 → 首次发布即 `UnsupportedTypeSupport: Could not import 'rosidl_typesupport_c'`，表现为 `cli_topic_delay` 的 `/ps`（PoseStamped）发布者悄悄崩溃、`topic delay` 报"topic does not appear to be published yet"。**排查法**：设备端遍历 site-packages 逐个 `import <pkg>.<pkg>_s__rosidl_typesupport_c` 找出炸的接口包（本次仅 `geometry_msgs` 一个）。**修复**：`build_ros2_package.sh src/ros2/common_interfaces/geometry_msgs` 重建（产物 C 生成/typesupport/fastrtps/introspection + Python 扩展全部带上新符号），打包重部署到两板的 **underlay 与 overlay 两处**（launcher 的 LD/PYTHONPATH overlay 在前，必须覆盖 overlay 副本）。**结论**：重建集合 = OHOS 改动 repo ∪ 消息集变化的接口 repo（`common_interfaces` 等）。
