...
Audit date: 2026-07-04; final Plan-A acceptance audit: 2026-07-13 EDT / 2026-07-14 CST
Change: `complete-rmw-mdds-full-parity-acceptance`

This matrix is the current acceptance boundary for the broad "perfect/all ROS 2 middleware features" goal. A
`proven` row has current code, test, contract, or board evidence. An `incomplete` row blocks production/full-feature
completion until it has a RED test/contract, implementation, runtime evidence, or explicit user acceptance as out
of scope. The final Plan-A audit accepts only the performance, target LeakSanitizer, conditional-skip, security,
and local-handoff boundaries stated below; it does not silently waive an unsupported RMW API.

2026-07-13 EDT / 2026-07-14 CST final Plan-A exact-artifact decision:

- The user approved the final Plan-A production profile. Every required row is now proven or has an explicit
  accepted boundary, so OpenSpec task 5.5 is complete. This decision supersedes lower current-completion wording;
  dated lower sections remain useful historical checkpoints.
- Both boards run production RMW `f8f74a1b...`, broker `4437466b...`, bridge `12d87b68...`, and SoftBus client
  `e1771298...`. The valid-domain sequential service soak is 36,000/36,000 over 7349.758/7362.085 seconds with
  zero timeout/error/residual. Domain 241 is invalid for the accepted 0..232 range and its partial run is marked
  `REJECTED`, not reused as evidence.
- The same artifacts pass a four-client exact-wire 16 MiB service soak for at least 7208 seconds per client:
  client sent/ok/valid and server request/valid counts are all 1,789, four DONE acknowledgements and all return
  codes are valid, failure counters are zero, and no worker, broker, or socket remains. An exact-key parser fix
  closed the short-smoke `valid`/`invalid` false-failure before the formal gate.
- Each board passes 16/16 programs and 129 assertions with exactly six capability-conditional skips. The three
  direct-loan conditions pass 3/3 without a broker; publisher/subscription allocation and serialized-size behavior
  pass 5/5 in the AArch64 capability supplement. These positive runs, not the skips alone, close the row.
- Fresh host package CTest is 24/24, the rmw_mdds upstream subset is 16/16, the delivery umbrella passes, and full
  functional `test_rmw_implementation` is 64/64. Uncrustify 0.83.0_f differs only on two clean upstream baseline
  files; the aggregate lint suite is not reported as 70/70 and the baseline is not changed for this delivery.
- The approved absolute 1 KiB performance threshold is p95 <= 50 ms and throughput >= 100 msg/s. rmw_mdds passes
  at 3.089 ms / 1774.141 msg/s; Fast DDS remains faster at 0.399 ms / 2482.394 msg/s and is retained as an
  optimization backlog, not an equivalence claim. OHOS LeakSanitizer is unavailable, so host leak detection and
  board invalid-access ASAN remain distinct and no target leak-clean claim is made. Required security is the
  proven signed-policy plus authenticated/encrypted protected-transport scope.
- Delivery is the previously approved committed local handoff. The final acceptance tooling is ROS commit
  `3b08223d043a74c6ed0d2606e1763556746ac78f`; no remote push, PR, or archive is implied.
...
| Service APIs | `rmw_create_service`, `rmw_create_client`, `rmw_send_request`, `rmw_take_request`, `rmw_send_response`, `rmw_take_response`, and availability checks are implemented; `test_service_inproc` covers AddTwoInts, Trigger, array service payloads, wait behavior, client cleanup, and pre-ACK delivery. Current artifacts passed 10/10 independent-process rounds at 50/50 with zero timeout/error/process-timeout, plus 50/50 `many_clients_one_process` and `one_client_many_requests` models. The independent exact-wire 16 MiB four-client soak passed 1,789/1,789 for at least 7208 seconds per client. | proven for service API, small-service 50-way, sequential soak, and concurrent-large soak | Keep unit, host CLI, native M2M, gateway service, structured stress, and both exact-artifact soak gates. |
...
| Host delivery | Current package CTest and upstream `rmw_mdds_cpp` pass 24/24 and 16/16. Full functional `test_rmw_implementation` passes 64/64. Full package ASAN with host leak detection and full package TSAN with halt-on-error each pass 24/24; dynamic publisher-loan ASAN/TSAN each pass 13/13. Runtime C/C++ and typesupport tests plus all scoped contracts pass, and the delivery umbrella reaches its final marker. System XML lint passes and cppcheck follows its upstream slow-version policy; Uncrustify 0.83.0_f differs only on two clean upstream baseline files. | proven for functionality and sanitizers; baseline lint-tool difference explicitly accepted | Keep package, upstream, full sanitizer, generated-memory-resource, typesupport, and delivery gates. Do not report the aggregate lint invocation as 70/70 or reformat unrelated upstream baselines for this delivery. |
| Board delivery | Both boards match production RMW `f8f74a1b...`, broker `4437466b...`, bridge `12d87b68...`, and SoftBus client `e1771298...`. This exact line passes P0/P1, all three 50-way models, QoS/types, native and gateway action, action-bag, signed/protected SROS2, exact 16 MiB topic/service repeat10, graph/action churn, current-source ASAN/TSAN, performance, 16/16 programs with 129 assertions plus positive skip supplements, the 36,000-call sequential soak, and the four-client 1,789-call exact-wire 16 MiB soak. | proven for the approved current production profile | Preserve exact hashes, board result artifacts, and accepted-boundary wording. Host HDC status 139 cannot replace or invalidate complete board markers. |
| P2/P3 production evidence gates | Functional conformance, signed protected security, action bag, all three service stress models, large-message, graph churn, host and board sanitizers, and performance thresholds are repeated on the current source/production line. Current production passes 30/30 50-way rounds, 36,000/36,000 sequential service calls, and 1,789/1,789 concurrent exact-wire 16 MiB calls for at least two hours with zero error/residual. The absolute 1 KiB gate passes; the Fast DDS relative gap remains disclosed. OHOS LeakSanitizer is unavailable and six conditional skips have positive supplements. | proven for the approved Plan-A production profile | Keep both soak gates and every explicit boundary in future release audits. Treat relative performance, target LeakSanitizer support, or broader security combinations as new scoped work, not silently expanded claims. |
| Delivery handoff | The user previously accepted committed local handoff with no push, PR, Gerrit submission, or OpenSpec archive. ROS 2 branch `jazzy-ubuntu-20.04` contains final acceptance tooling commit `3b08223d043a74c6ed0d2606e1763556746ac78f`; intended documentation is committed separately without staging unrelated generated files. DSoftBus/MDDS remains on its scoped local branch. | proven for the selected local handoff endpoint | Keep future commits scoped and do not absorb unrelated dirty-worktree changes. Remote publication remains a separate user action. |
...
The user-approved Plan-A `rmw_mdds_cpp`/MDDS production profile is **complete**. Current exact artifacts carry the
full functional, conformance, stress, QoS/type, action/bag, signed protected-security, large-message, graph,
sanitizer, performance, sequential-soak, and concurrent-large-soak evidence summarized above. The six upstream
conditional skips are closed by positive direct-loan and capability supplements rather than by waiver.

Completion retains explicit wording boundaries: rmw_mdds is not performance-equivalent to Fast DDS; target
LeakSanitizer is unavailable and no board leak-clean claim is made; security certification is limited to the
approved signed and authenticated/encrypted transport profile; new message shapes or hypothetical security
combinations require their own RED scopes. Within those accepted boundaries, no required matrix or local-handoff
work remains and Task 5.5 is complete. Lower NOT-COMPLETE statements describe their dated artifacts only.
